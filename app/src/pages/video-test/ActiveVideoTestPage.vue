<template>
  <q-page v-if="activeTestExist" padding>
    <q-card>
      ...
      <q-stepper v-model="step" vertical color="primary" animated>
        ...
        >
          <div style="width: 100%" v-if="step === index">
            <q-item
              style="
                display: flex;
            ...
            <q-item
              v-for="(answers, index2) in question.answers"
              :key="answers"
              dense
  ...
  <q-page v-else class="row items-center justify-evenly">
    ...
    >
      ...
      <q-card-section
        style="align-self: center; display: flex; flex-direction: column"
      >
        <q-card-section class="text-center text-bold" style="font-size: 16px">
          Trenutno ne postoji aktivan test! Pokušajte ponovo kasnije.
        </q-card-section>
...
function nextStep(index: number) {
  ...
  if (showConfirmationDialog.value) {
    $q.dialog({
      ...
      message:
        'Ukoliko odete na naredno pitanje, više se ne možete vratiti na ovo pitanje.',
      options: {
        type: 'checkbox',
        model: [],
        items: [
          {
            label: 'Ne prikazuj ovaj dialog više',
            value: '1',
            color: 'secondary',
          },
        ],
...
function validationSuccessful(questionIndex: number) {
  const answerIsNotSelected = questions.value[questionIndex].answers.every(
